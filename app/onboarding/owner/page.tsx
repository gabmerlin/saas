...
export default function OwnerOnboardingPage() {
  ...
  async function onPublish() {
    ...
    if (!r1.ok) {
      if (r1.status === 409) {
        ...
        if (errorData.error === "AGENCY_ALREADY_EXISTS") {
          ...
          setTimeout(() => {
            router.push('/onboarding/onboarding-blocked');
          }, 3000);
        } else {
    ...
    const r2 = await fetch("/api/onboarding", {
      method: "POST",
      headers: { 
        "content-type": "application/json",
        "authorization": `Bearer ${authToken}`,
    ...
    {currentTenantId && selectedPlan && (
      <BTCPayPopup
        ...
        instagramAddon={adv.instagramAddon}
          ...
          try {
            // Attendre un délai pour laisser le temps à l'agence d'être créée
            await new Promise(resolve => setTimeout(resolve, 3000));
            
            // Vérifier que l'agence existe avant de rediriger
            const checkAgencyResponse = await fetch(`/api/agency/status?subdomain=${basic.subdomain}`);
            const agencyData = await checkAgencyResponse.json();
            
            if (agencyData.ok && agencyData.agency) {
              // Rediriger vers le dashboard de l'agence
              const rootDomain = process.env.NEXT_PUBLIC_ROOT_DOMAIN || 'qgchatting.com';
              const target = `https://${basic.subdomain}.${rootDomain}/dashboard`;
